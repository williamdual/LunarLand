...
{
    if (DEBUG)
        UtilityFunctions::print("Enter Tree - BuildingObj.");
}

void BuildingObj::_ready()
{
    if (DEBUG)
        UtilityFunctions::print("Ready - BuildingObj.");
}
...
void BuildingObj::SetValues(int building_type, bool is_textured, bool auto_collision)
{
    ...
    UtilityFunctions::print(vformat("%s%s%s", "Textures/", texture_names[building_type], texture_formats[building_type]));
